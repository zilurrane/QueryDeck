...
- **P2-14** ✅ Multi-engine support — driver/dialect abstraction in `db.rs` (`AnyConn`); **SQL Server (`tiberius`) + PostgreSQL, MySQL/MariaDB, and SQLite (`sqlx`)** all shipped, each verified against a live database. Follow-ups: dialect-aware inline editing (currently MSSQL-only), column metadata for empty server result sets, multi-statement scripts
