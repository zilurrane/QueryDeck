...
- **P2-14** 🟡 Multi-engine support — driver/dialect abstraction in `db.rs` (`AnyConn`); **PostgreSQL and MySQL/MariaDB shipped** via `sqlx` alongside `tiberius`. Next: **SQLite** (same `sqlx` crate). Follow-ups: dialect-aware inline editing (currently MSSQL-only), column metadata for empty Postgres/MySQL result sets, multi-statement scripts
